...
def region_ADD_shap_sum(tb_log_dir):
    ...
    shap_dir = '/gemini/code/ncomms2022-main/shap_mid/'
...
def region_COG_shap_sum(tb_log_dir):
    shap_dir = '/root/autodl-tmp/shap_mid/'
    array_NC, array_MCI, array_DE, count_NC, count_MCI, count_DE = [], [], [], 0, 0, 0
    for i in range(5):
        with open(tb_log_dir + 'cross{}/'.format(i) + 'test_eval.csv', 'r') as csv_file:
            reader = csv.DictReader(csv_file)
            for row in reader:
                seg_dir = '/root/autodl-tmp/seg/' + row['filename'].replace('.npy', '.nii')
                if not os.path.exists(seg_dir):
                    continue
                if row['COG'] in ['0', '0.0'] and row['COG_pred'] in ['0', '0.0']:
                    seg = nib.load(seg_dir).get_fdata()[6:-6, 6:-6, 6:-6]
...
def shap_region_scores_to_csv(data_dir, task):
    ...
    for file in glob(seg_dir + '*.nii'):
        ...
        seg = nib.load(file).get_fdata()[6:-6, 6:-6, 6:-6]
...
if __name__ == "__main__":
    ...
    # shap_region_scores_to_csv('/root/autodl-tmp/ADNI_neuroPath/', 'DE')

...
def plot_all(task, stains, layers):
    ...
    with open('../NeuroPathTable/ALL.csv', 'r') as csv_file:
        ...
        for row in reader:
            if row['dataset'] in ['ADNI'] and row['diff_days'] and int(row['diff_days']) < 365 * 2:
                mri_id_map[row['id']] = '/root/autodl-tmp/ADNI_processed_npy/'.format(row['dataset']) + row['filename']
                shap_id_map[row['id']] = ['/root/autodl-tmp/ncomms2022-main/shap/shap_{}_'.format(row['dataset'], layer, task) + row['filename'] for layer in layers]
                seg_id_map[row['id']] = '/root/autodl-tmp/seg/'.format(row['dataset']) + row['filename'].replace('.npy', '.nii')

...
class Fusion_Model_Wrapper():
    ...
    def get_optimal_thres(self, csv_name='valid'):
        ...
        for i, task in enumerate(self.tasks):
            if task == 'COG' and self.config['COG']['type'] == 'reg':
                thres['NC'], thres['DE'] = COG_thresholding(self.tb_log_dir + csv_name + '_mri_eval.csv')
            elif task == 'ADD':
                thres[task] = ADD_thresholding(self.tb_log_dir + csv_name + '_mri_eval.csv')
            else:
                print("optimal for the task {} is not supported yet".format(task))
        return thres
    ...
    def gen_score(self, stages=['train', 'valid', 'test', 'OASIS'], thres={'ADD':0.5, 'NC':0.5, 'DE':1.5}):
        for stage in stages:
            data = pd.read_csv(self.csv_dir + stage + '_mri.csv')[self.features + self.tasks + ['filename']]
            data = self.drop_cases_without_label(data, 'COG')
            COG_data = self.preprocess_pipeline(data[self.features+['COG']], 'COG') # treat it as COG data to do the preprocessing
            features = COG_data.drop(['COG'], axis=1)
            labels = data[self.tasks]
            ...
            with open(self.tb_log_dir + stage + '_mri_eval.csv', 'w') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=list(content[0].keys()))
                writer.writeheader()
                for case in content:
                    writer.writerow(case)
    ...
    def shap(self, stage='test'):
        ...
        """
        # get the data ready
        data = pd.read_csv(self.csv_dir + stage + '_mri.csv')
        task_data = []
        for task in self.tasks:
            task_data.append(data[self.features + [task]])
        for i, task in enumerate(self.tasks):
...
class Fusion_Trans_Wrapper():
    ...
    def gen_score(self, stages=['train', 'valid', 'test', 'OASIS'], thres={'ADD':0.5, 'NC':0.5, 'DE':1.5}):
        for stage in stages:
            data = pd.read_csv(self.csv_dir + stage + '_mri_trans.csv')[self.features + self.tasks + ['filename']]
            data = self.drop_cases_without_label(data, 'COG')
            COG_data = self.preprocess_pipeline(data[self.features+['COG']], 'COG') # treat it as COG data to do the preprocessing
            features = COG_data.drop(['COG'], axis=1)
            labels = data[self.tasks]
    ...
    def shap(self, stage='test'):
        ...
        """
        # get the data ready
        data = pd.read_csv(self.csv_dir + stage + '_mri_trans.csv')
        task_data = []
        for task in self.tasks:
            task_data.append(data[self.features + [task]])
        for i, task in enumerate(self.tasks):
    ...
    def load_preprocess_data(self):
        data_train = pd.read_csv(self.csv_dir + '_mri_trans.csv')
        for task in self.tasks:
            self.train_data.append(data_train[self.features + [task]])
        for i, task in enumerate(self.tasks):
            self.train_data[i] = self.preprocess_pipeline(self.train_data[i], task)

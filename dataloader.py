...
class Resize3D(object):
    ...
    def __call__(self, img):
        resized_img = F.interpolate(torch.tensor(img).unsqueeze(0).unsqueeze(0), size=self.size, mode='trilinear', align_corners=False)
        return resized_img.squeeze(0).squeeze(0).numpy()

# Set data loading and transformation
desired_depth = 182
desired_height = 218
desired_width = 182
transform = transforms.Compose([
    Resize3D((desired_depth, desired_height, desired_width)),
    transforms.ToTensor(),
])
    ...
    with open(filename, 'r') as csvfile:
        ...
        for row in reader:
            ...
            if config['type'] == 'reg':
                ...
                label_list.append(int(float(row[task])))
            else:
